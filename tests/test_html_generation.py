...
def test_code_block_conversion(example_html_soup):
    ...
    code_title = example_html_soup.find(
        "div", class_="code-title", string="Hello World Example"
    )
    ...
    assert (
        code_block_with_linenums is not None
    ), "Code block with line numbers not found"
...
def test_link_conversion(example_html_soup):
    ...
    assert (
        example_html_soup.find(
            "a", href="https://github.com/hkopenai", string="HKOpenAI"
        )
        is not None
    )
