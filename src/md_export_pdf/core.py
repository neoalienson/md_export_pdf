...
class MarkdownToPdfConverter:
    def __init__(
        ...
        logger.debug(
            f"Initializing MarkdownToPdfConverter with input_file={input_file}, output_file={output_file}, css_file={css_file}"
        )
    ...
    def convert(self):
        logger.info(
            f"Starting conversion of '{self.input_file}' to '{self.output_file}'"
        )
        ...
        if self.cover_page_file:
            ...
            if cover_page_md_content:
                ...
                logger.warning(
                    f"Cover page file not found or empty: {self.cover_page_file}"
                )
        ...
        if self.header_content:
            ...
            if header_md_content:
                md = markdown.Markdown(
                    extensions=["extra", "codehilite", "attr_list", "tables"]
                )
        ...
        if cover_page_html:
            cover_story = fitz.Story(
                html=cover_page_html, user_css=combined_css_content
            )
        ...
        while more:
            ...
            if header_html:
                header_story = fitz.Story(
                    html=header_html, user_css=combined_css_content
                )
            ...
            if footer_html:
                ...
                current_footer_html = current_footer_html.replace(
                    "{total_pages}", "{total_pages_placeholder}"
                )

                footer_story = fitz.Story(
                    html=current_footer_html, user_css=combined_css_content
                )
                footer_story.place(
                    fitz.Rect(
                        0, page_rect.height - margin, page_rect.width, page_rect.height
                    )
                )
        ...
        for i in range(doc.page_count):
            ...
            if footer_html:
                current_footer_html = footer_html.replace(
                    "{page_num}", str(current_content_page_num)
                )
                current_footer_html = current_footer_html.replace(
                    "{total_pages}", str(total_pages_final)
                )
                ...
                page.insert_htmlbox(
                    footer_rect, current_footer_html, css=combined_css_content
                )
        ...
        logger.info(
            f"Successfully converted '{self.input_file}' to '{self.output_file}'"
        )
